...
void GoToNextScene::OnCollisionEnter(CCollider* _pOther)
{
	...
	if (pOtherObj->GetName() == L"PLAYER")
	{
		...
		if (pCurScene && pCurScene->GetName() == L"Stage4")		///현재씬을 받아와서 다음 씬으로 넘기기위한 조건입니다.
		{
			CSceneManager::GetInstance()->ChangeScene(SCENE_TYPE::STAGE_05); ///자~~ 다음 씬 드가자~~~~~~~~~~
		}
		if (pCurScene && pCurScene->GetName() == L"Stage5")		///현재씬을 받아와서 다음 씬으로 넘기기위한 조건입니다.
		{
			CSceneManager::GetInstance()->ChangeScene(SCENE_TYPE::STAGE_06); ///자~~ 다음 씬 드가자~~~~~~~~~~
		}
		if (pCurScene && pCurScene->GetName() == L"Stage6")		///현재씬을 받아와서 다음 씬으로 넘기기위한 조건입니다.
		{
			CSceneManager::GetInstance()->ChangeScene(SCENE_TYPE::STAGE_07); ///자~~ 다음 씬 드가자~~~~~~~~~~
		}
	}
}
